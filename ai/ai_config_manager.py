...
管理 AI 决策相关的配置，包括技术指标、时间周期、提示词预设等
"""
import json
import sqlite3
...
class AIConfigManager:
    ...
    def _ensure_table(self):
        ...
        try:
            ...
            # 尝试添加 model 列（如果表已存在但没有该列）
            try:
                cursor.execute("ALTER TABLE ai_api_configs ADD COLUMN model TEXT DEFAULT ''")
    ...
    def get_ai_api_config(self, ai_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT api_key, enabled, verified, model FROM ai_api_configs WHERE ai_id = ?",
                (ai_id,)
            )
            row = cursor.fetchone()
            conn.close()
    ...
    def save_ai_api_config(self, ai_id: str, api_key: str, enabled: bool = True, verified: bool = False, model: str = "") -> bool:
        """保存 AI API 配置"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            cursor.execute("""
                INSERT INTO ai_api_configs (ai_id, api_key, model, enabled, verified, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(ai_id) DO UPDATE SET
                    ...
                    updated_at = excluded.updated_at
            """, (ai_id, api_key, model, 1 if enabled else 0, 1 if verified else 0, now, now))
